...
import { buildPathOptions, buildSeamAllowanceOptions, boundingBoxToPx, calculateFitToScreen } from './canvasUtils'
...
export default function CanvasArea() {
  ...
  const fitToScreen = useCallback(() => {
    ...
    const { zoom, panX, panY } = calculateFitToScreen(bboxPx, canvas.getWidth(), canvas.getHeight())
  ...
  useEffect(() => {
    ...
    measurementLines.forEach((yCm) => {
      ...
      const line = new fabric.Line([(minOffsetX - 10) * CM_TO_PX, yPx, (maxOffsetX + 10) * CM_TO_PX, yPx],
    ...
    pieces.forEach((piece) => {
      ...
      if (firstPoint) {
        const label = new fabric.Text(`قطعة ${piece.piece_index + 1}${piece.is_flipped ? ' (معكوسة)' : ''}`,
    ...
    <div className="relative flex-1 h-full bg-gray-50 canvas-container-ltr">
      ...
      {pieces.length > 0 && (
        <button onClick={fitToScreen}
          className="absolute bottom-4 left-4 bg-white shadow-lg border border-gray-200 rounded-lg px-4 py-2 text-sm font-medium text-gray-700 hover:bg-blue-50 hover:border-pattern-blue hover:text-pattern-blue transition-colors z-10">
          ⊡ ملائمة الشاشة
        </button>
      )}
      {isLoading && (
      ...
      {error && (
        <div className="absolute top-4 left-4 bg-red-50 border border-red-200 text-red-700 rounded-lg px-4 py-2 text-sm">{error}</div>
      )}
      {pieces.length === 0 && !isLoading && (
        <div className="absolute inset-0 flex items-center justify-center text-gray-400 text-sm text-center px-4">
          غيّر أي مقاس من القائمة على اليمين لعرض الباترون
        </div>
      )}
    </div>
  )
